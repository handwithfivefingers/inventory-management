...
const PAYMENT_TYPES = (t: Function) => [
...
export const OrderForm = ({
  ...
  // Line total incl. per-line VAT: base (buyPrice snapshot, excl. VAT) x (1 + lineVAT / 100).
  // Lines without their own VAT fall back to the header VAT (defaultTax).
  const lineRateOf = (item: OrderDetailSchema) => Number((item as any)?.VAT ?? VAT ?? 0);
  const lineBaseOf = (item: OrderDetailSchema) =>
  ...
  const total = (orderDetails || []).reduce(
    (sum, item: OrderDetailSchema) => sum + lineBaseOf(item) * (1 + lineRateOf(item) / 100),
    0,
  );
  const combineTotal = total + Number(surcharge || 0);
  const totalPaid = Number(combineTotal + (combineTotal / 100) * Number(VAT || 0));
      ...
      <form className="flex gap-2 flex-col" onSubmit={form.handleSubmit(onHandleSubmit, onError)}>
        <div className="relative">
          <TextInput
            ref={scanInputRef}
            // inputSize="sm"
            prefix={<Icon name="search" className="w-4 text-slate-400" />}
            placeholder="Tìm theo Barcode/SKU/Name"
            value={scanValue}
            autoComplete="off"
        ...
        products.map((product) => {
          ...
          return (
            <div
              key={product.id}
              className={cn(
                "border-b border-slate-100 last:border-b-0 dark:border-slate-700 flex gap-2 p-2 w-full",
                "flex flex-col",
              )}
            >
              <div
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => productPickable && onSelect(product)}
                className={cn("w-full flex items-center gap-3 px-3 py-2 text-left transition-colors", {
                  // ["hover:bg-indigo-50 dark:hover:bg-slate-700"]: productPickable,
                  ["opacity-45 cursor-not-allowed"]: !productPickable,
                })}
              >
                <div className="block relative w-5">
                  <span className="pb-[100%] block" />
                  <Icon name="shopping-cart" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                </div>
              ...
              {/* {product.variants?.map((variant) => {
                ...
                return (
                  ...
                  >
                    <div className="block relative w-5">
                      <span className="pb-[100%] block" />
                      <Icon name="sliders" className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div
                      className={cn("w-16 shrink-0 text-right text-xs", stock <= 0 ? "text-red-500" : "text-slate-500")}
                    >
                      {stock}
                    </div>
                  </button>
