...
export const MultiSelectInput = forwardRef<HTMLDivElement, IMultiSelectInput & actions>(
  (
    {
      label,
      name,
        ...
        >
          ...
          >
            {rest.value?.map((item, index: number) => {
              ...
              return (
                <TagSelection
                  onClose={() => removeOptions(item)}
                  key={`option-${item}-${index}`}
                  label={selected.get(item)?.label}
                />
              );
            }) || placeholder}
          </div>
...
const TagSelection = ({ onClose, label }: { onClose: () => void; label: string }) => {
  return (
    <div className="bg-primary/5 pl-1 pr-2 flex gap-1 rounded overflow-hidden border border-slate-700/10">
      <Icon
        name="x"
        fontSize={12}
        strokeWidth={3}
        className="text-primary/70 hover:text-primary shrink-0 w-3"
        onClick={(e: React.MouseEvent<ChangeEvent>) => {
          e.preventDefault();
          onClose();
        }}
      />
      <span className="text-slate-500 truncate text-[12px]">{label}</span>
    </div>
  );
};
