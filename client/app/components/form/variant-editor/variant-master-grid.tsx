...
export const VariantMasterGrid = ({ fields, units, attributes = [], remove }: VariantMasterGrid) => {
  ...
  const nameOf = (variant: IVariantDraft) => {
    return (
        ...
        return (
          ...
          >
                  ...
                  <SelectInput
                    ...
                    onSelect={(value) =>
                      form.setValue(
                        ...
                        { ...(variant.options || {}), [attribute.name]: String(value) },
                        { shouldDirty: true },
                      )
                    }
                  />
            ...
            <PermissionGuard
              ...
              fallback={<span className="text-sm">{variant.skuCode || "—"}</span>}
            >
              <TextInput
                inputSize="xs"
                aria-label="SKU"
                value={String(variant.skuCode || "")}
                onChange={(event) =>
                  form.setValue(`variants.${index}.skuCode`, event.target.value, {
                    shouldDirty: true,
                    shouldValidate: true,
                  })
                }
            ...
            <div>
              {variant.variantId ? (
                ...
                <PermissionGuard
                  permission="UPDATE"
                  module={MODULE_ENUM.product}
                  fallback={<span className="font-medium">{quantity}</span>}
                >
                  <NumberInput
                    aria-label={t("product.stock", { defaultValue: "Tồn kho ban đầu" })}
                    type="number"
                    min="0"
                    className="w-20 rounded border border-slate-300 px-2 py-1 text-sm"
                    value={quantity.toString()}
                    onValueChange={(v) =>
                      form.setValue(`variants.${index}.quantity`, v.value, {
                        shouldValidate: true,
                        shouldDirty: true,
                      })
                    }
              ...
              {quantity === 0 && <Badge variant="warn">{t("product.outOfStock", { defaultValue: "Hết hàng" })}</Badge>}
            </div>
            <div className="flex items-center justify-end gap-1">
              <PricingUnitDrawer
                index={index}
                units={units}
                trigger={t("product.configurePricing", { defaultValue: "Cấu hình Giá & Đơn vị" })}
              />
              <PermissionGuard permission="UPDATE" module={MODULE_ENUM.product}>
                <SwitchInput
                  checked={variant.isActive !== false}
                  onChange={(event: any) =>
                    form.setValue(`variants.${index}.isActive`, event.target.checked, { shouldDirty: true })
                  }
                />
              </PermissionGuard>
              <PermissionGuard permission="UPDATE" module={MODULE_ENUM.product}>
...
VariantMasterGrid.displayName = "VariantMasterGrid";
