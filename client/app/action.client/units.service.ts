...
const unitsService = {
  get: (searchParams: ICategoryQueryParams): Promise<IResponse<IUnit[] | undefined>> => {
  ...
  getById: ({ id }: { id: string | number }): Promise<IResponse<IUnit | undefined>> => {
    const params = new URLSearchParams({});
    return http.get(API_PATH.units + "/" + id + "?" + params.toString());
  },
  update: ({ id, ...params }: IUnitParams): Promise<IResponse<IUnit[] | undefined>> => {
