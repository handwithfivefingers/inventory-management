    ...
    rows.forEach((row, index) => {
      ...
      if (!Number.isInteger(conversionRate) || conversionRate <= 0) {
        ctx.addIssue({
          ...
          path: [index, "conversionRate"],
          message: "Conversion rate must be a positive integer",
        });
      }
      const duplicateUnitAt = rows.findIndex(
        (candidate, candidateIndex) => candidateIndex < index && String(candidate.unitId) === String(row.unitId),
      );
      if (row.unitId != null && duplicateUnitAt !== -1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [index, "unitId"],
          message: "This unit is already used in this variant",
        });
      }
      const barcode = String(row.barcode || "")
        .trim()
        .toUpperCase();
      const duplicateBarcodeAt = barcode
        ? rows.findIndex(
      ...
      if (duplicateBarcodeAt !== -1) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [index, "barcode"],
          message: "Barcode is duplicated in this product payload",
        });
      }
      ...
      if (Number.isFinite(cost) && Number.isFinite(retail) && retail < cost)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [index, "retailPrice"],
          message: "Retail price must be at least cost price",
        });
      if (Number.isFinite(wholesale) && Number.isFinite(retail) && wholesale > retail)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: [index, "wholesalePrice"],
    ...
    (product.variants || []).forEach((variant, variantIndex) => {
      (variant.barcodes || []).forEach((row, barcodeIndex) => {
        ...
        if (seen.has(barcode)) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["variants", variantIndex, "barcodes", barcodeIndex, "barcode"],
            message: "Barcode is duplicated in this product payload",
          });
        }
        seen.add(barcode);
...
export type BarcodeRowsSchemaType = z.infer<typeof barcodeRowsSchema>;
export type VariantOverrideSchemaType = z.infer<typeof variantOverrideSchema>;
