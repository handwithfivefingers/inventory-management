...
module.exports = class WarehouseController {
  ...
  async get(req, res, next) {
    try {
      const { count, rows } = await new WarehouseService().getWarehouse(req);
  ...
  async activeWarehouse(req, res, next) {
    try {
      const resp = await new WarehouseService().activeWarehouse(req);
      return res.status(200).json({
        data: resp,
      });
    } catch (error) {
