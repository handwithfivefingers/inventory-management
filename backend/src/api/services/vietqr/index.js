/**
 * STRUCTURE Viet QR CODE
 *
 * [0] Payload Format Indicator | ID = 00 | Length = 02 | Value = 01
 * [1] Method | ID = 01 | Length = 02 | Value = 11
 * [2] Consumer Account Information | ID = 38 | Length <= 99 |
 * [3] Merchant Category Code | ID = 52 | Length = "04" |
 * [4] Transaction Currency | ID = 53 | Length = "03" |
 * [5] Transaction Amount | ID = 54 | Length <= 13 |
 * [6] Tip or Convenience Indicator | ID = 55 | Length = 02 |
 * [7] Value of Convenience Fee Fixed | ID = 56 | Length = 02 |
 */
  ...
  getCrc16_array(text, hex_output = true) {
    ...
    let result = text.map((row) =>
      row.map((string) => {
        ...
        const bytes = Array.from(String(string)).map((char) => char.charCodeAt(0) & 0xff); // gives 8 bits; higher bits get discarded
        let crc = 0xffff;
        bytes.forEach((byte) => {
          for (let i = 0; i < 8; i++) {
...
class VIETQRService {
  ...
  async generateQR() {
    try {
      const vietQRData = {
        bankCode: "970436", // Example bank code
        accountNumber: "TRUYENMAI",
        amount: "100000", // Amount in VND (optional)
        message: "Tesing Payment for services",
      };

      const qrArr = [];
      const payloadFormat = new TLV(0, "Payload Format Indicator", 2, true, "M", "01");
      const QRType = new TLV(1, "QR Type", 2, true, "M", this.fields.is_dynamic_qr ? "12" : "11");
      const accountInfo = new TLV(38, "QR code service on NAPAS system", 99, false, "M", [
        new TLV(0, "Global Unique Identifier - GUID", 10, true, "M", NAPAS_GUID),
        new TLV(1, "Payment network specific", 32, false, "M", [
          new TLV(0, "Acquier ID/BNB ID", 6, true, "M", vietQRData.bankCode),
          new TLV(1, "Merchant ID/Consumer ID", 19, false, "M", vietQRData.accountNumber),
        ]),
        new TLV(2, "Service Code", 10, false, "C", this.fields.service_code),
      ]);
      const merchantCateCode = new TLV(52, "Merchant Category Code", 4, true, "O", this.fields.merchant_category);
      const transactionCurrency = new TLV(53, "Transaction Currency", 3, true, "M", this.fields.currency);
      const transactionAmount = new TLV(54, "Transaction Amount", 13, false, "C", vietQRData.amount);

      qrArr.push(payloadFormat);
      qrArr.push(QRType);
      qrArr.push(accountInfo);
      qrArr.push(merchantCateCode);
      qrArr.push(transactionCurrency);
      qrArr.push(transactionAmount);

      qrArr.push(new TLV(55, "Tip or Convenience Indicator", 2, true, "O"));
      qrArr.push(new TLV(56, "Value of Convenience Fee Fixed", 13, false, "O"));
      qrArr.push(new TLV(57, "Value of Convenience Fee Percentage", 5, false, "O"));
      qrArr.push(new TLV(58, "Country Code", 2, true, "M", this.fields.country_code));
      qrArr.push(new TLV(59, "Merchant Name", 25, false, "O", this.fields.merchant_name));
      qrArr.push(new TLV(60, "Merchant City", 15, false, "O", this.fields.merchant_city));
      qrArr.push(new TLV(61, "Postal Code", 10, false, "O", this.fields.postal_code));
      qrArr.push(
        new TLV(62, "Additional Data Field Template", 99, true, "O", [
          null,
          new TLV(1, "Bill Number", 25, false, "C", this.fields.bill_number),
          new TLV(2, "Mobile Number", 25, false, "C", this.fields.mobile_number),
          new TLV(3, "Store Label", 25, false, "O", this.fields.store_label),
          ...
          new TLV(6, "Customer Label", 25, false, "C", this.fields.customer_label),
          new TLV(7, "Terminal Label", 25, false, "O", this.fields.terminal_label),
          new TLV(8, "Purpose of Transaction", 25, false, "C", CRC.nonAccentVietnamese(vietQRData.message)),
          new TLV(9, "Additional Consumer Data Request", 3, false, "O", this.fields.additional_data),
        ])
      );
      qrArr.push(new TLV(63, "CRC (Cyclic Redundancy Check)", 4, true, "M"));
      qrArr.push(
        new TLV(64, "Merchant Information - Language Template", 2, true, "O", [
          new TLV(0, "Language Preference", 2, true, "M", this.fields.lang_ref),
          new TLV(1, "Merchant Name - Alternate Language", 25, false, "M", this.fields.local_merchant_name),
          new TLV(2, "Merchant City - Alternate Language", 15, false, "O", this.fields.local_merchant_city),
        ])
      ...
      qrArr.push(
        new TLV(80, "UniCube data", 99, false, "0", [
          new TLV(0, "uuid", 16, false, "O", this.fields.uuid),
          new TLV(1, "custom data", 83, false, "O", this.fields.custom_data),
        ])
      );
      ...
      let semi_vietqr = `${str}6304`;
      let crc_value = CRC.getCrc16_array(semi_vietqr);
      console.log("crc_value", crc_value);
      console.log("str", str);
      const strQR = `${semi_vietqr}${crc_value}`;
      const qrString = await QRCODE.toDataURL(strQR);
      //   const qrString = qrArr.join("");
      //   console.log("qrArr", JSON.stringify(qrArr, null, 2));
      return qrString;
    } catch (error) {
      throw error;
    }
