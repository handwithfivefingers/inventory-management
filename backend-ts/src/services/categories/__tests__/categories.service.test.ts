...
describe('CategoriesService', () => {
  ...
  describe('create', () => {
    it('throws when name is missing', async () => {
      await expect(service.create({ vendorId: 1 } as any)).rejects.toThrow('Category name is required')
    })

    it('throws when vendorId is missing', async () => {
      await expect(service.create({ name: 'Cat' } as any)).rejects.toThrow('Vendor is required')
    })
    ...
    it('rethrows when saving fails', async () => {
      ...
      await expect(service.create({ name: 'Cat', vendorId: 2 } as any)).rejects.toThrow('db down')
    })
  })
  ...
  describe('update', () => {
    it('updates a category by id and returns the result', async () => {
      ...
      expect(database.category.update).toHaveBeenCalledWith({ name: 'New' }, { where: { id: 1 } })
    ...
    it('rethrows when the update fails', async () => {
      ...
      await expect(service.update({ id: 1, name: 'New' } as any)).rejects.toThrow('db down')
    })
  })

  describe('getCategories', () => {
    it('throws when vendorId is missing', async () => {
      await expect(service.getCategories({ limit: 10, offset: 0 } as any)).rejects.toThrow('Vendor is required')
    })
    ...
    it('rethrows when the query fails', async () => {
      ...
      await expect(service.getCategories({ vendorId: 'v1' } as any)).rejects.toThrow('db down')
    })
  })
