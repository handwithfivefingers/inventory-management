...
describe("FinancialService.getReport - PROCESS 4 Functional Testing", () => {
  ...
  beforeEach(() => {
    ...
    db.invoice.findAll.mockResolvedValue([] as any);
  });
  ...
  it("filters by warehouseId and transactionDate range", async () => {
    ...
    const invoiceWhere = db.invoice.findAll.mock.calls[0][0].where;
    expect(invoiceWhere.warehouseId).toBe(2);
    expect(invoiceWhere.createdAt[Op.gte]).toEqual(new Date("2026-08-01T00:00:00.000"));
  });
  ...
  it("clamps netRevenue to 0 when vat exceeds revenue", async () => {
    mockSums(100, 0, 0);
    db.invoice.findAll.mockResolvedValue([{ taxAmount: 10000 }] as any); // vat 10000 > revenue 100
    const res = await service.getReport({}, null);
    expect(res.netRevenue).toBe(0);
  });
...
describe("FinancialService.getReport - PROCESS 5 Stress Testing", () => {
  ...
  it("handles negative VAT edge (should not produce negative vatCollected to inflate netRevenue)", async () => {
    mockSums(1000, 0, 0);
    db.invoice.findAll.mockResolvedValue([{ taxAmount: -100 }] as any);
    const res = await service.getReport({}, null);
  ...
  it("distinguishes issued/paid invoices for VAT (only issued/paid counted)", async () => {
    mockSums(0, 0, 0);
    db.invoice.findAll.mockResolvedValue([] as any);
    await service.getReport({}, null);
    const invoiceWhere = db.invoice.findAll.mock.calls[0][0].where;
    expect(invoiceWhere.status[Op.in]).toEqual(['issued', 'paid']);
  });
  ...
  it("handles concurrent large order arrays for VAT calc performance", async () => {
    mockSums(0, 0, 0);
    const manyInvoices = Array.from({ length: 10000 }, () => ({ taxAmount: 10 }));
    db.invoice.findAll.mockResolvedValue(manyInvoices as any);
    const start = Date.now();
    const res = await service.getReport({}, null);
    expect(res.vatCollected).toBe(10000 * 10); // 10 per invoice
