...
describe("CustomerService", () => {
  ...
  describe("getCustomers", () => {
    ...
    it("rethrows when the query fails", async () => {
      ...
      await expect(
        service.getCustomers({ query: {} } as any),
  ...
  describe("getCustomerById", () => {
    it("returns the customer found by id", async () => {
      ...
      expect(database.customer.findByPk).toHaveBeenCalledWith(
        7,
        expect.objectContaining({ include: expect.arrayContaining([database.vendor]) }),
      );
    });
    ...
    it("throws when the customer is not found", async () => {
      ...
      await expect(
        service.getCustomerById({ params: { id: 99 } } as any),
    ...
    it("rethrows when the query fails", async () => {
      ...
      await expect(
        service.getCustomerById({ params: { id: 7 } } as any),
      ).rejects.toThrow("db down");
    });
  });
  ...
  describe("create", () => {
    ...
    it("creates a customer within a transaction and commits", async () => {
      ...
      expect(database.customer.create).toHaveBeenCalledWith(
        expect.objectContaining({ name: "Acme", vendorId: "v1" }),
        { transaction: tx },
      );
    ...
    it("rolls back when creation fails", async () => {
      ...
      await expect(
        service.create({ body: { name: "Acme", vendorId: "v1" } } as any),
      ).rejects.toThrow("db down");
  ...
  describe("delete", () => {
    it("throws when the customer is not found", async () => {
      ...
      await expect(
        service.delete({ params: { id: 7 } } as any),
      ).rejects.toThrow("Customer not found");
    });
    ...
    it("throws when the customer has invoices", async () => {
      ...
      await expect(
        service.delete({ params: { id: 7 }, user: { vendorId: "v1" } } as any),
      ).rejects.toThrow("Cannot delete customer with existing invoices");
    });
